...
// Standard ESM resolution of __dirname and __filename
...
You MUST populate the sourceUrl and sourceName fields with real, verified URLs and publisher names (e.g. Vanguard, Premium Times, Punch, etc.) retrieved from your search grounding results.`;
    ...
    console.error("Error fetching briefings from Gemini:", error);
    // Gracefully fallback to high-quality default briefings so the app never fails
    return res.json({ briefings: defaultBriefings, source: "Local Intelligence Database (Fallback)" });
  }
});
...
app.post("/api/chat", async (req, res) => {
  ...
  const getFallbackResponse = (msg: string): string => {
    ...
    if (text.includes("lagos") || text.includes("ikeja")) {
      return `📍 *Anti-Risk Lagos Headquarters & Command Center*\n` +
    ...
    if (text.includes("abuja") || text.includes("garki")) {
      return `📍 *Anti-Risk Abuja Federal Capital Territory Command*\n` +
    ...
    if (text.includes("calabar") || text.includes("cross river")) {
      return `📍 *Anti-Risk Calabar South-South Regional Command*\n` +
    ...
    if (text.includes("oyo") || text.includes("ibadan")) {
      return `📍 *Anti-Risk Oyo Western Command Sector Office*\n` +
    ...
    if (text.includes("abia") || text.includes("aba")) {
      return `📍 *Anti-Risk Abia Eastern Region Command Hub*\n` +
    ...
    if (text.includes("phone") || text.includes("contact") || text.includes("call") || text.includes("number") || text.includes("email") || text.includes("hotline")) {
      return `📞 *Anti-Risk Security Hotline & Dispatch Directory*\n\n` +
    ...
    if (text.includes("address") || text.includes("location") || text.includes("where") || text.includes("office") || text.includes("hub") || text.includes("station")) {
      return `🏢 *Anti-Risk Physical Command Hub Directory*\n\n` +
    ...
    if (text.includes("guard") || text.includes("patrol") || text.includes("sentinel") || text.includes("hire") || text.includes("force")) {
      return `🛡️ *Anti-Risk Guard Force Operations*\n\n` +
    ...
    if (text.includes("mopol") || text.includes("escort") || text.includes("bodyguard") || text.includes("vip") || text.includes("convoy") || text.includes("transit") || text.includes("close protection")) {
      return `🚔 *Anti-Risk Armed Escort & VIP Close Protection*\n\n` +
    ...
    if (text.includes("cctv") || text.includes("camera") || text.includes("surveillance") || text.includes("alarm") || text.includes("sensor")) {
      return `📹 *AI-Powered CCTV & Electronic Surveillance Networks*\n\n` +
    ...
    if (text.includes("dog") || text.includes("k9") || text.includes("k-9") || text.includes("sniffer")) {
      return `🐕 *Anti-Risk K-9 Guard & Scent Detection Division*\n\n` +
    ...
    if (text.includes("training") || text.includes("academy") || text.includes("recruit") || text.includes("job") || text.includes("apply") || text.includes("career")) {
      return `🎓 *Anti-Risk Security Training Academy & Careers*\n\n` +
    ...
    if (text.includes("price") || text.includes("cost") || text.includes("how much") || text.includes("rate") || text.includes("fee") || text.includes("quote")) {
      return `💰 *Anti-Risk Custom Security Quotations & Pricing*\n\n` +
    ...
    if (text.includes("hello") || text.includes("hi ") || text.includes("hey") || text.includes("good day") || text.includes("morning") || text.includes("afternoon") || text.includes("evening")) {
      return `Good day and a warm welcome to the Anti-Risk Security Command Center. I am Officer Grace, your customer support dispatcher.\n\n` +
    ...
    return `Thank you for contacting the Anti-Risk Security Command Desk. I am Officer Grace, and I want to make sure you get the exact information you need.\n\n` +
   ...
   try {
     ...
     if (!ai) {
       ...
       return res.json({ response: getFallbackResponse(message), source: "Officer Grace | Dispatch" });
     }
     ...
     const prompt = `You are Officer Grace, an elite customer support dispatcher at Anti-Risk Security Services in Nigeria. 
 Your tone must be professional, polite, highly reassuring, vigilant, and structured. 
 
 SERVICES TO PROMOTE:
 1. Routine Guarding (100% fingerprint vetted, RFID patrol clocking, NSCDC-compliant).
 2. Special Operations (Close Protection Officers, armed convoy with Mobile Police MOPOL integration, VIP transits).
 3. CCTV & AI Surveillance (night-vision cameras, thermal imaging, automatic laser tripwires, remote monitoring control rooms).
 4. Security Dogs (K-9 Division: German Shepherds, Belgian Malinois, Rottweilers, certified explosive detection handlers).
 5. Equipment & Assets Guarding (cash-in-transit, biometric access gates).
 6. Safety Training Academy (NSCDC-compliant physical combat, fire safety, and emergency CPR courses in Lagos).
 7. Advisory & Physical vulnerability audits.
 8. Custom operations (maritime security escorts, political rally security).
 ...
 GOAL & INSTRUCTIONS:
 - You must be EXTREMELY INFORMATIVE, helpful, and descriptive. Give direct, concrete, detailed, and highly specific answers about our addresses, phone numbers, emails, pricing, vetting processes, training curriculum, and commanders.
 - NEVER give vague, evasive, or extremely brief responses. If the user asks for contact information, a phone number, or an address, give them the exact, formatted directory details immediately.
 - Use bullet points, bold headers, and structured lists so your answers are clean, scannable, and extremely professional.
 - Speak in a reassuring, authoritative, yet polite Nigerian-corporate tone (e.g., 'Good day', 'Highly welcome', 'Sir/Ma').
 - Urge the user to fill out the 'Quote Calculator' or the 'Contact' form for a formal security assessment or pricing, but ONLY after fully answering their query with concrete facts first.
 - Avoid any system code, technical variables, or larping. Be a genuine, expert dispatcher.
 ...
 Please generate your response.`;
     ...
     if (responseText && responseText.trim()) {
       return res.json({ response: responseText.trim(), source: "Officer Grace | Dispatch" });
     } else {
       throw new Error("Received empty response text from Gemini.");
     }
 
   } catch (error) {
     console.error("Error generating support response via Gemini:", error);
     return res.json({ response: getFallbackResponse(message), source: "Officer Grace | Dispatch" });
   }
 });
